...
function AddEducationalDetail({ navigation }) {
    ...
    async function addEducationDetail(board, level, subjects, year, institution) {
        let data = { image, board, level, subjects, year, institution }
        if (image) {
            await AsyncStorage.setItem("educationdetail" + uuid.v4(), JSON.stringify(data))
    ...
    return (
        ...
        <SafeAreaView style={styles.outerMostView}>
            ...
            <Formik
                ...
                onSubmit={(values) => (
                    addEducationDetail(board = values.board, level = values.level, subjects = values.subjects, year = values.year, institution = values.institution)
                )}
            >
                {({
                    values,
...
export default AddEducationalDetail